...
$term_desc  = term_description() ? wp_strip_all_tags( term_description() ) : sprintf( 'Seluruh kegiatan pelatihan Cleanique Academy dalam kategori %s.', $term_title );
?>
...
<section class="about-hero-section" style="background-image: linear-gradient(145deg, rgba(7, 35, 56, 0.94) 0%, rgba(11, 93, 143, 0.90) 60%, rgba(8, 127, 193, 0.86) 100%), url('<?php echo esc_url( get_template_directory_uri() . '/assets/images/hero-lab-practical.jpg' ); ?>');">
    <div class="container about-hero-container">
        <div class="hero-editorial-pill">
            ...
            <span>JENIS KEGIATAN &bull; <?php echo esc_html( strtoupper( $term_title ) ); ?></span>
        </div>
        ...
        if ( ! is_wp_error( $all_terms ) && ! empty( $all_terms ) ) : ?>
            <div style="display: flex; gap: 0.6rem; flex-wrap: wrap; margin-bottom: 2.75rem; justify-content: center;">
                <a href="<?php echo esc_url( home_url( '/dokumentasi-event/' ) ); ?>"
                   ...
                   style="padding: 0.45rem 1.1rem; font-size: 0.85rem; border-radius: var(--radius-full); <?php echo ! is_tax() ? 'background: var(--color-primary); color: #fff; border-color: var(--color-primary);' : ''; ?>">
                    Semua
                </a>
                <?php foreach ( $all_terms as $t ) : ?>
                    <a href="<?php echo esc_url( get_term_link( $t ) ); ?>"
                       class="btn btn-outline"
                       style="padding: 0.45rem 1.1rem; font-size: 0.85rem; border-radius: var(--radius-full); <?php echo ( is_tax( 'jenis_kegiatan', $t->slug ) ) ? 'background: var(--color-primary); color: #fff; border-color: var(--color-primary);' : ''; ?>">
        ...
        <div class="grid grid-3" style="margin-bottom: 3.5rem; gap: 1.5rem;">
            ...
            if ( have_posts() ) :
                ...
                <div style="text-align: center; grid-column: 1/-1; padding: 3.5rem 0; background: #ffffff; border-radius: var(--radius-lg); border: 1px solid var(--color-border);">
                    <p style="color: var(--color-text-muted); margin-bottom: 1.5rem; font-size: 1.05rem;">Belum ada kegiatan dalam kategori ini.</p>
